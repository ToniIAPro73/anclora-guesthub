...
Anclora GuestHub captures controlled pilot requests and authenticates approved pilot users. It does not decide access.

Flow:

1. Public landing asks for `Solicitar piloto controlado`.
2. `POST /api/pilot/request` validates the request and forwards it to Nexus with `NEXUS_GUESTHUB_WEBHOOK_SECRET`.
3. If Nexus is unavailable, GuestHub falls back to an internal Resend notification when configured.
...
7. GuestHub validates email/password against `PilotUser.passwordHash` and stores a signed httpOnly session cookie with role.
